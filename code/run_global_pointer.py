...
class GaiicDataset(DatasetBase):
    ...
    def create_examples(self, data: Any, data_type: str, **kwargs) -> List[Dict[str, Any]]:
        ...
        for (i, line) in enumerate(data):
            ...
            examples.append(dict(guid=guid, tokens=tokens, entities=entities))
        return examples
...
def load_data(data_name, data_dir, data_type, tokenizer, max_sequence_length, **kwargs):
    process_piplines = [
        ProcessExample2Feature(
            GaiicDataset.label2id(), tokenizer, max_sequence_length),
    ]
...
def main():
    ...
    metrics = [SequenceLabelingScore(GaiicDataset.get_labels(),schema='BIOS',average= 'micro')]
