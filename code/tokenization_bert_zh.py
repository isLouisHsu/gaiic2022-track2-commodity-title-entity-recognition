    ...
    def tokenize(self, text: TextInput, **kwargs) -> List[str]:
        ...
        for token in tokens:
            ...
            if token in no_split_token:
                ...
                tokenized_offset_mapping.append([0, 0])
            else:
                text, offset_mapping = self._tokenize(token)
                tokenized_text.extend(text)
                tokenized_offset_mapping.extend(offset_mapping)
        # ["This", " is", " something", "<special_token_1>", "else"]
        return tokenized_text, tokenized_offset_mapping
