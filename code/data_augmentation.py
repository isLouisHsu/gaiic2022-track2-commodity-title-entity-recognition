...
if __name__ == '__main__':
    ...
    augment_pipline = [
        AugmentRandomMask(p=0.5, mask_mode="non-entity", mask_proba=0.15, mask_token="[MASK]"
            ) if args.do_random_mask_augment else None,
        AugmentExchangeEntity(p=0.3) if args.do_exchange_entity_augment else None,
        AugmentExchangeSegments(p=0.2, beta=5) if args.do_exchange_segments_augment else None,
    ]
    augmented_examples = []
    ...
    # for example, augmented in zip(examples, augmented_examples):
    #     print()
