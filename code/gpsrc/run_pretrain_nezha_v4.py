...
record_save_path = '../data/public_data'
...
class GaiicTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss1 = outputs1[0]
        loss2 = outputs2[0]
        active_indexs = inputs['attention_mask'].view(-1) == 1

        loss3 = rdrop_loss_fun(outputs1[1].view(-1, outputs1[1].size(-1))[active_indexs],
                               outputs2[1].view(-1, outputs2[1].size(-1))[active_indexs])
        loss_weight = (1 - 0.3) / 2
        loss = loss_weight * loss1 + loss_weight * loss2 + 0.3 * loss3
